...
// 👉 Paste your Instagram Reel link here, e.g. "https://www.instagram.com/reel/XXXXXXXXXXX/"
const REEL_URL = "https://www.instagram.com/reel/DZ8sASMufl3/?igsh=MW9xMjR3N2tzb3o4aA==";

const STORY_DURATION = 2800; // ms each photo stays on screen
...
function buildStory() {
  ...
  reelSlide.className = "story-slide reel-slide";
  reelSlide.innerHTML = `
    <h3>One more thing 🎬</h3>
    <div class="reel-embed-wrap" id="reel-embed-wrap"></div>
    <button id="story-finish">Continue to the site 🎉</button>
  `;
  storyStage.appendChild(reelSlide);
  storySlides.push(reelSlide);
}

function loadReelEmbed() {
  const wrap = document.getElementById("reel-embed-wrap");
  if (!wrap) return;

  if (!REEL_URL) {
    wrap.innerHTML = `<p style="color:#fff; text-align:center; font-size:0.9rem; opacity:0.8;">Add your Instagram Reel link to REEL_URL in script.js to show it here.</p>`;
    return;
  }

  wrap.innerHTML = `<blockquote class="instagram-media" data-instgrm-permalink="${REEL_URL}" data-instgrm-version="14" style="margin:0 auto; max-width:400px; width:100%;"></blockquote>`;
...
function goToStorySlide(i) {
  ...
  if (isReel) {
    loadReelEmbed();
    // no auto-advance here — visitor taps "Continue" when ready
  } else {
